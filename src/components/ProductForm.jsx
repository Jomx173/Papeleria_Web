...
function ProductForm({ initialProduct, onSubmit }) {
  ...
  return (
    <form className="product-form" onSubmit={handleSubmit} noValidate>
      ...
      <label>
        Nombre
        <input type="text" name="nombre" value={form.nombre} onChange={handleChange} placeholder="Nombre del producto" autoFocus />
      ...
      <label>
        Cantidad
        <input type="number" name="cantidad" min="0" step="1" value={form.cantidad} onChange={handleChange} placeholder="0" />
        {errors.cantidad && <span className="field-error">{errors.cantidad}</span>}
      </label>

      <label>
        Precio
        <input type="number" name="precio" min="0.01" step="0.01" value={form.precio} onChange={handleChange} placeholder="0.00" />
        {errors.precio && <span className="field-error">{errors.precio}</span>}
      </label>

      <label>
        Stock mínimo
        <input type="number" name="stock_minimo" min="0" step="1" value={form.stock_minimo} onChange={handleChange} placeholder="5" />
      ...
      <div className="form-actions">
        ...
        <button type="submit">{initialProduct ? "Guardar cambios" : "Registrar producto"}</button>
      </div>
    </form>
  );
}
